...
struct hapn {
	...
	uint16 bufsiz;           /* Maximum RX frame size              */
	uint8 *rcp;		/* RX data pointer                    */

	struct mbuf *sndq;      /* Transmit frames queue              */
	uint16 sndcnt;           /* Count of frames on sndq            */
...
#define CSMA 0
#define FULLDUP 1
	uint16 base;             /* Base I/O address of board          */
	unsigned vec;           /* Interrupt level                    */
	INTERRUPT (*oldvec)(void);	/* Previous interrupt service vector */
...
/* In hapn.c: */
INTERRUPT (far *(haint)(int dev))();

/* In hapnvec.asm: */
INTERRUPT ha0vec(void);
